"""POST /ingest/wazuh — receives Wazuh integrator/webhook payloads."""
...
from ..sources import wazuh as wazuh_parser
...
@router.post(
    ...
    summary="Wazuh alert webhook",
    description=(
        "Accepts a single Wazuh alert JSON document or a list of them, "
        "normalizes each to ECS, and forwards to the configured sink."
    ),
...
async def ingest_wazuh(
    ...
    for i, p in enumerate(payloads):
        try:
            parsed.append(wazuh_parser.parse(p))
